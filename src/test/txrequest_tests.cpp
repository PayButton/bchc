...
struct Runner
{
    ...
    /** Which (peer, txid) combinations are known to be expired. These need to be accumulated here instead of
...
public:
    ...
    void CheckExpired(NodeId peer, TxId txid)
    {
        ...
        auto& runner = m_runner;
        runner.actions.emplace_back(m_now, [=,&runner]() {
            auto it = runner.expired.find(std::make_pair(peer, txid));
...
void BuildWtxidTest(Scenario& scenario, int config)
{
    ...
    if (config & 2) {
        ...
        scenario.CheckExpired(peerW, txid);
    }
